...
function UF:CreateClassPowerBar(parent, max, cpType, level)
    local bar = _G.CreateFrame("Frame", "$parent"..strgsub(cpType, "_", "").."Bar", parent)
    ...
    for i = 1, max do
        ...
        texture:SetVertexColor(unpack(M.colors.power[bar.__type]))
        ...
        glow:SetVertexColor(unpack(M.colors.power[bar.__type.."_GLOW"]))
        glow:SetAlpha(0)
        element.Glow = glow
    ...
    bar.UpdateTexture = function() end
...
local function OverrideStaggerBar(self, event, unit)
    ...
    bar:SetMinMaxSmoothedValue(0, maxHealth)
    bar:SetSmoothedValue(stagger)
...
local function StaggerBar_OnHide(self)
    UF:Reskin(self:GetParent(), "NONE", true, 0, self.__type)
end
...
function UF:CreateStaggerBar(parent, level)
    ...
    _G.Mixin(bar, _G.SmoothStatusBarMixin)
